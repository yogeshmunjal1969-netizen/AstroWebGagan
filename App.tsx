...
import { User, Native, Service, Appointment, AppointmentStatus, BlackoutDate, ConsultationMode } from './types';
import { api } from './services/mockData';
import { 
  Sun, Moon, Calendar, User as UserIcon, LogOut, 
  Menu, X, CheckCircle, XCircle, Clock, MapPin, 
  ShieldCheck, AlertCircle, Plus, ChevronRight, Video,
  LayoutDashboard, Users, Briefcase, Settings as SettingsIcon, CalendarOff, Trash2, Edit2, Database,
  Tag, Timer, Image as ImageIcon, CheckCircle2, ListPlus,
...
const NativeForm = ({ onSubmit, onCancel, existingNatives }: any) => {
...
const BookingFlow = ({ service, client, onSuccess, onCancel }: any) => {
  ...
  useEffect(() => {
    setNatives(api.getNatives(client.id));
  ...
  const generateSlotsForDate = (selectedDateStr: string) => {
    ...
    slots.forEach(slot => {
      ...
      for (const appt of dateAppts) {
        ...
        const apptService = api.getServices().find(s => s.id === appt.serviceId);
...
const AdminDashboard = () => {
  ...
  const [calendarConfig, setCalendarConfig] = useState(api.getCalendarConfig());
  ...
  const refreshData = () => {
    setAppointments([...api.getAppointments('admin-1', 'ADMIN')]);
    setServices([...api.getServices()]);
    setClients([...api.getAllClients()]);
    setBlackoutDates([...api.getBlackoutDates()]);
    setCalendarConfig(api.getCalendarConfig());
  };
        ...
        <Card className="animate-fade-in">
           ...
           <div className="space-y-6">
             {clients.map(client => {
                ...
                const natives = api.getNatives(client.id);
                return (
...
const ClientDashboard = ({ user }: { user: User }) => {
  ...
  useEffect(() => {
    setAppointments(api.getAppointments(user.id, 'CLIENT'));
  }, [user.id]);
...
const App = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (isRegistering) {
       ...
       const foundUser = await api.login(loginPhone, 'password');
       if (foundUser) {
         setUser(foundUser);
         setPage('home');
       } else {
